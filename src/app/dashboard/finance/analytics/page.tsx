              ...
              <div style={{ ...cardStyle, marginBottom: "2rem", border: "1px solid var(--primary-glow-border)", background: "linear-gradient(135deg, var(--primary-glow), var(--accent-glow))" }}>
                ...
                {aiLoading ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    {[1, 2, 3].map((i) => (
                      <div key={i} style={{ height: "60px", borderRadius: "10px", background: "rgba(255,255,255,0.03)", animation: "pulse 1.5s infinite" }} />
                    ))}
                  </div>
                ) : aiInsights.length > 0 ? (
                    ...
                    {aiInsights.map((insight, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem" }}>
                          ...
                          <span style={{ fontWeight: 700, fontSize: "0.9rem" }}>{insight.title}</span>
                        </div>
                        <p style={{ margin: 0, fontSize: "0.82rem", color: "rgba(255,255,255,0.7)", lineHeight: 1.5 }}>{insight.description}</p>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <div className="finance-tools-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "2rem" }}>
                ...
                <div className="projections-card" style={cardStyle}>
                  <div className="card-header-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
                    ...
                    <span style={{ fontSize: "0.8rem", padding: "0.2rem 0.5rem", borderRadius: "6px", background: "rgba(168,85,247,0.15)", color: "#a855f7", fontWeight: 600 }}>
                      Growth: {projectionGrowthRate > 0 ? "+" : ""}{projectionGrowthRate}%
                    </span>
                  </div>
                  <p style={{ margin: "0 0 1.25rem 0", fontSize: "0.8rem", color: "rgba(255,255,255,0.6)" }}>
                    Linear regression forecast for the next 3 months. Drag the slider to simulate revenue growth.
                  </p>
                      ...
                      <div key={idx} className="projection-item-row" style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: "0.75rem", color: "var(--text-light)" }}>
                            Rev: <span style={{ color: "#10b981" }}>₹{item.revenue.toLocaleString()}</span> · Exp: <span style={{ color: "#ef233c" }}>₹{item.expenses.toLocaleString()}</span>
                          </div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                ...
                <div className="break-even-card" style={cardStyle}>
                  ...
                  <div className="break-even-inputs-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.75rem", marginBottom: "1.25rem" }}>
                    <div>
                      ...
                      <input 
                        ...
                        style={{ ...inputStyle, width: "100%", padding: "0.4rem 0.6rem", fontSize: "0.8rem" }}
                      />
                    </div>
                    <div>
                      <label style={{ ...labelStyle, fontSize: "0.75rem" }}>Price / Tree (₹)</label>
                      <input 
                        type="number" 
                        value={pricePerTreeBE} 
                        onChange={(e) => setPricePerTreeBE(Math.max(1, Number(e.target.value)))}
                        style={{ ...inputStyle, width: "100%", padding: "0.4rem 0.6rem", fontSize: "0.8rem" }}
                      />
                    </div>
                    <div>
                      <label style={{ ...labelStyle, fontSize: "0.75rem" }}>Labor / Tree (₹)</label>
                      <input 
                        type="number" 
                        value={workerCostPerTreeBE} 
                        onChange={(e) => setWorkerCostPerTreeBE(Math.max(0, Number(e.target.value)))}
                        style={{ ...inputStyle, width: "100%", padding: "0.4rem 0.6rem", fontSize: "0.8rem" }}
                      />
                    </div>
                  </div>
                  ...
                  {(() => {
                    const marginPerTree = pricePerTreeBE - workerCostPerTreeBE;
                    const breakEvenTrees = marginPerTree > 0 ? Math.ceil(fixedCosts / marginPerTree) : Infinity;
                    const currentTrees = filteredJobs.reduce((s, j) => s + (j.trees || 0), 0);
                    const progressPct = breakEvenTrees > 0 && breakEvenTrees !== Infinity ? Math.min(100, Math.round((currentTrees / breakEvenTrees) * 100)) : 0;
                    const remainingTrees = Math.max(0, breakEvenTrees - currentTrees);
                    ...
                    return (
                      <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                        <div className="break-even-summary-row" style={{
                          ...
                          <div>
                            <div style={{ fontSize: "0.75rem", color: "var(--text-light)" }}>Variable Profit Margin</div>
                            <div style={{ fontWeight: 700, fontSize: "1rem", color: "#10b981" }}>₹{marginPerTree} per tree</div>
                          </div>
                          <div style={{ textAlign: "right" }}>
                            <div style={{ fontSize: "0.75rem", color: "var(--text-light)" }}>Break-Even Target</div>
                            <div style={{ fontWeight: 700, fontSize: "1rem", color: "var(--foreground)" }}>
                              {breakEvenTrees === Infinity ? "N/A" : `${breakEvenTrees} trees`}
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", marginBottom: "0.35rem" }}>
                            <span>Progress ({currentTrees} / {breakEvenTrees === Infinity ? "∞" : breakEvenTrees} trees)</span>
                          ...
                          <div style={{ height: "8px", background: "var(--surface-2)", borderRadius: "4px", overflow: "hidden" }}>
                            <div style={{
                              width: `${progressPct}%`, height: "100%", background: progressPct >= 100 ? "#10b981" : "#f59e0b",
                              borderRadius: "4px", transition: "width 0.5s ease"
                            }} />
                          </div>
                          <p style={{ margin: "0.5rem 0 0 0", fontSize: "0.75rem", color: "var(--text-dim)" }}>
                            {remainingTrees > 0 
                              ? `Need to harvest ${remainingTrees} more trees this period to cover fixed costs.`
                              : "Break-even point achieved! Net revenue is now accumulating profit."}
                          </p>
                        </div>
                      </div>
                    );
